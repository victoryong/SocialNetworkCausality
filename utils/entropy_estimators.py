...
def cmi(x, y, z, k=3, base=2):
    ...
    tree = ss.cKDTree(points)
    dvec = [tree.query(point, k + 1, p=float('inf'))[0][k] for point in points]
    a, b, c, d = avgdigamma(list(zip2(x, z)), dvec), avgdigamma(list(zip2(y, z)), dvec), avgdigamma(z, dvec), digamma(k)
    return (-a - b + c + d) / log(base)
...
def midd(x, y, base=2):
    """ Discrete mutual information estimator
        Given a list of samples which can be any hashable object
    """
    return -entropyd(list(zip(x, y)), base) + entropyd(x, base) + entropyd(y, base)
...
def centropyd(x, y, base=2):
  """ The classic K-L k-nearest neighbor continuous entropy estimator for the
      entropy of X conditioned on Y.
  """
  return entropyd(list(zip(x, y)), base) - entropyd(y, base)
